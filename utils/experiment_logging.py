...
class ExperimentHelper():
    ...
    def __init__(self, args, results_dir='gran', train=True):
        ...
        with open(self.__config_file_name, 'w') as f:
            ...
            self.logger.info(json.dumps(self.args, indent=4))
    ...
    def save_results(self, dict_res, iteration=None):
        """
        ...
        if iteration is None:
            self.append_results(dict_res)
        else:
            self.insert_results(dict_res, iteration=iteration)
    ...
    def append_results(self, dict_res):
        results = self.load_results()
    ...
    def checkpoint_model_if_best(self, dict_res, to_save, epoch, criteria, objective):
        """
        ...
        if self.new_score_is_better(new_score):
            ...
            self.checkpoint_model(to_save, epoch, best=True)
            return True
        return False

    def checkpoint_model(self, to_save, epoch, best=False):
        """
        Checkpoints the given model/optimizer/scheduler to run_dir/checkpoints
        Uses the scheduler to obtain the current epoch.
        """
        filename = os.path.join(self.run_dir, 'models', 'epoch_{:07d}.h5'.format(epoch)) if not best else \
                os.path.join(self.run_dir, 'best_model_epoch_{:07d}.h5'.format(epoch))

        torch.save(to_save, filename)

    def log(self, *args):
        string = ' '.join([str(temp) for temp in args])
